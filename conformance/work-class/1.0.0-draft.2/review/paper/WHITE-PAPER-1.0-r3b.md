<!-- Revision 3 draft built from WHITE-PAPER-1.0-r2.md (source SHA-256 90e6cc74275fc452a3a26e699c01a90864add7e71f44a3090a3549c41ca8853d) by feathering draft 2 material into its existing structure. Supersedes the assembled draft WHITE-PAPER-1.0-r3.md as the working revision. -->

# Governance Belongs to the Work

**The Work-Class Governance Specification for Consequential Agentic Systems**

*Seampoint, open review candidate, paper revision 3, September 14, 2026*

## Summary

An agent can hold a valid credential, call an approved tool and follow its instructions while carrying out an unauthorized business act. The missing permission may concern the exact object, the amount, the person who approved it, the age of the evidence or exposure already created by other work. After dispatch, a timeout can leave the effect unknown. A plausible explanation and a successful API call do not settle those questions. Delegated work creates the problem: a prediction machine can choose and act, but cannot bear the authority or accountability an institution attaches to the act. Those relationships must map back to people and stay bound to the work itself.

A work class is a portable contract for one bounded kind of work, intended for independent implementation and technical review. It names the operations and objects in scope, the authority each operation requires, the constraints and aggregate limits that apply, the legal state transitions, the execution limits and the evidence that establishes completion. A work instance is one case under one exact class revision. That instance carries its authority, exposure, evidence and completion through every call, retry and participant change.

A conforming consumer takes explicit artifacts, observations, events and state and returns a deterministic judgment: admit, refuse, hold or report an unresolved condition according to the profile. A host commits the returned transition, enforces the decision at native dispatch and reports effect evidence back through the same identity. This division lets existing identity services, policy engines, workflow tools, databases and connectors operate as usual while binding their records to one contract.

The standard includes four composition profiles: linear work, choice and bounded loops, activity deadlines, and structured parallel work with frozen fan-out. It defines typed constraints, direct business authority, bounded aggregate evaluation, shared reservations and evidence readback. Repeated expansion passes, dynamic fan-out, compensation, general cancellation and distributed execution are explicitly excluded. Business-day calendars, signed netting and implicit currency conversion are also outside the candidate. Specification candidate `1.0.0-draft.2`, called draft 2 below, adds three capabilities the first candidate lacked: deployment requires accepted review records bound to the exact definition, a deadline can run from the completion of a named earlier step, and an authority refusal names each check that failed. Draft numbers identify specification candidates; paper revision numbers identify editions of this paper.

Candidate `seampoint.work-class/1.0.0-draft.2`, frozen at pin `5442ee3b`, has 80 requirements and 425 frozen cases carrying 296 contract judgments, 295 of them as exact vectors. Expected results were derived from the text before either consumer ran. TypeScript and Python consumers with no shared evaluator reproduce all 425 byte for byte. Four synthetic application specimens, in access, money, care and force, run end to end under both, and a self-contained export reproduces the suite without the source repository. One independent review round examined the draft 2 changes; its findings were fixed or decided, and those corrections have not been independently reviewed. The evidence does not qualify a production deployment, an authentic observation provider, a person’s identity, durable storage, native effect delivery or distributed coordination, and no role is qualified.

This open review candidate is intended for technical review of precision, sufficiency, independent implementability and test strength. It does not claim universal coverage, comparative superiority, production readiness, regulatory approval or external endorsement.

Paper revision 3 answers the first round of technical review. Reviewers asked whether a translated policy carries any human attestation, what a host must actually do, whether a runtime exists, and whether one contract can hold across domains without a vocabulary for each. Section 4.1 describes the attestation records draft 2 requires, section 5 maps host obligations onto controls enterprises already run, section 6 reports the draft 2 evidence and the runtime kernel, section 7 runs four unlike domains through the same contract, and section 8 describes the role of authoring and how an independent reviewer inspects a candidate.

## 1. Why consequential agentic work is hard to govern
...
## 2. What existing controls accomplish, and where their contracts stop
...
## 3. Requirements for deterministic work-class governance
...
### 4.1 Attestation before deployment
...
## 5. Runtime placement and host obligations
...
### 5.1 Host integration and the enterprise baseline
...
### 5.2 Why the host does not have to be perfect
...
## 6. Evidence for specification candidate `1.0.0-draft.2`

Every figure in this section comes from the draft 2 freeze record at pin `sha256:5442ee3b55675d849e388981487b806c0be29f686df6df8520b488508f86ceec`. Draft 1’s results stay attached to draft 1.
...
### 7.3 Inpatient sepsis surveillance
...
## 8. How to use and review the standard
